      ...
      <section className="rounded-[18px] border border-[var(--app-border)] bg-[var(--app-shell)] p-3 shadow-2xl shadow-black/20 sm:p-4">
        <div className="rounded-[14px] border border-[var(--app-border)] bg-[var(--app-card)] p-5 shadow-lg shadow-black/15 sm:p-6">
          ...
          <div className="mt-6 flex min-h-[420px] items-center justify-center rounded-[14px] border-2 border-dashed border-[var(--app-border-strong)] bg-[var(--app-bark)] px-6 text-center text-[var(--app-text-muted)]">
            Upload a plant photo to see a polished care card with name,
            confidence, traits, and practical notes.
          </div>
        </div>
      </section>
    );
    ...
    <section className="rounded-[18px] border border-[var(--app-border)] bg-[var(--app-shell)] p-3 shadow-2xl shadow-black/20 sm:p-4">
      <div className="rounded-[14px] border border-[var(--app-border)] bg-[var(--app-panel-soft)] p-5 text-[var(--app-ink)] shadow-lg shadow-black/15">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-4xl font-black tracking-tight text-[var(--app-ink)]">
              {result.commonName}
            </h2>
            <p className="mt-1 text-lg italic text-[var(--app-ink-muted)]">
        ...
        {lowConfidence && (
          <div className="mt-5 rounded-[12px] border border-amber-300 bg-amber-50 p-4 text-sm leading-relaxed text-amber-900">
            Low-confidence identification. Compare the alternatives below and
            confirm with another source before acting on the result.
          </div>
        )}

        <p className="mt-5 text-base leading-relaxed text-[var(--app-ink)]">
          {result.identificationNotes}
        </p>
        <p className="mt-4 text-sm leading-relaxed text-[var(--app-ink-muted)]">
          AI-assisted identification can be uncertain. Confirm before eating or
          touching unknown plants, treating pests or disease, or exposing pets
          and children.
        </p>
        ...
        <div className="mt-5 rounded-[14px] border border-[var(--app-border)] bg-[var(--app-card)] p-5 shadow-lg shadow-black/15">
          ...
          <div className="mt-3 grid gap-3">
            {result.likelyAlternatives.map((plant) => (
              <div
                key={`${plant.commonName}-${plant.scientificName}`}
                className="rounded-[12px] border border-[var(--app-border)] bg-[var(--app-bark)] p-4"
              >
      ...
      <div className="mt-5 grid gap-3">
        ...
        {Object.entries(resultSectionLabels).map(([key, label]) => {
          ...
          if (!text) return null;
          return (
            <CareDetails key={key} title={label} open={key === "overview"}>
              {text}
            </CareDetails>
          );
        })}
      ...
      <div className="mt-4 grid gap-3">
        {conditionItems.map(([title, text]) => (
          <div key={title} className="rounded-[12px] bg-slate-100 p-4">
            ...
            <p className="mt-2 text-sm leading-relaxed text-[var(--app-ink-muted)]">
              {text}
            </p>
          </div>
        ))}
...
function CareDetails({ title, open = false, children }) {
  return (
    <details
      className="group rounded-[12px] border border-[var(--app-border)] bg-[var(--app-panel-soft)] p-4 text-[var(--app-ink)] shadow-sm"
      open={open}
    >
      <summary className="cursor-pointer list-none text-base font-black text-[var(--app-ink)]">
        <SummaryLabel title={title} />
      </summary>
      <p className="mt-3 text-sm leading-relaxed text-[var(--app-ink-muted)]">
        {children}
      </p>
    </details>
  );
...
function SummaryLabel({ title }) {
  return (
    <span className="inline-flex w-full items-center justify-between gap-4">
      {title}
      <span className="rounded-full bg-slate-200 px-2 py-1 text-xs text-[var(--app-ink-muted)] group-open:hidden">
        Open
      </span>
      <span className="hidden rounded-full bg-slate-200 px-2 py-1 text-xs text-[var(--app-ink-muted)] group-open:inline">
        Close
      </span>
    </span>
  );
