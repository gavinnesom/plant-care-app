...
import { ACCEPTED_IMAGE_TYPES, MAX_IDENTIFICATION_IMAGES, MAX_IMAGE_BYTES } from '../lib/plantSchema';
...
export function UploadPanel({ photos, loading, onFilesSelected, onRemovePhoto, onIdentify, onClear, error }) {
    ...
    <section className="rounded-[18px] border border-[var(--app-border)] bg-[var(--app-shell)] p-3 shadow-2xl shadow-black/20 sm:p-4">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-3">
            <h2 className="text-2xl font-black text-[var(--app-text)]">Build a photo set</h2>
        ...
        {hasPhotos && <p className="max-w-full truncate rounded-full bg-emerald-300/10 px-3 py-1 text-sm font-bold text-emerald-100 ring-1 ring-emerald-300/25">{photos.length} of {MAX_IDENTIFICATION_IMAGES} photos</p>}
      </div>
      ...
      >
        <p className="max-w-2xl text-sm leading-relaxed text-[var(--app-text-muted)]">
          Drag plant photos here, or use Browse photos above. JPG, PNG, or WebP up to {maxMb} MB each.
        </p>
      </div>
      ...
      <div className="mt-5 overflow-hidden rounded-[14px] border border-[var(--app-border-strong)] bg-[var(--app-panel-soft)] shadow-sm">
        {hasPhotos ? (
          <div className="grid gap-3 p-3 sm:grid-cols-2">
            {photos.map((photo) => (
              <figure key={photo.id} className="relative overflow-hidden rounded-[12px] border border-[var(--app-border)] bg-[var(--app-bark)]">
                ...
                <figcaption className="flex items-center justify-between gap-2 px-3 py-2 text-xs font-bold text-[var(--app-text-muted)]">
                  ...
                  <button type="button" className="text-[var(--app-leaf)] hover:text-[#d5f0a5]" onClick={() => onRemovePhoto(photo.id)}>
                    Remove
                  </button>
                </figcaption>
              </figure>
