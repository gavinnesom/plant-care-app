...
function parseMultipartImages(contentType, body) {
  ...
  for (const part of parts) {
    ...
    if ((name === 'image' || name === 'images' || name === 'photos') && filename) {
...
function validateIdentificationImages(images) {
  ...
  if (images.length > MAX_IDENTIFICATION_IMAGES) {
    const error = new Error(`Choose no more than ${MAX_IDENTIFICATION_IMAGES} photos.`);
    error.statusCode = 400;
    throw error;
  }

  const totalBytes = images.reduce((sum, image) => sum + (image.buffer?.length || 0), 0);
