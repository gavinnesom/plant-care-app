...
module.exports = async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    if (session.configError) return res.status(session.configError.status).json(session.configError.body);
    return res.status(200).json({ unlocked: session.ok, expiresAt: session.expiresAt || null });
  }
